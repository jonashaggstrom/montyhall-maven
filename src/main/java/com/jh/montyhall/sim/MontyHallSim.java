...
public class MontyHallSim {
	...
	public Boolean compete(MontyHallContestant p) throws Exception {
		...
		guess = p.guess(boxList);
		if (AppConfig.debugoutput)
			System.out.println("Player: " + p.getName() + " noOfBoxes: " + boxList.size() + " last guess: " + guess
					+ " prizeBox " + prizeBox);
	...
	private void removeEmtyBox(int playersBox) {
		boxList.stream().filter(e -> e != playersBox && e != prizeBox).findFirst().ifPresent(boxList::remove);
	}
